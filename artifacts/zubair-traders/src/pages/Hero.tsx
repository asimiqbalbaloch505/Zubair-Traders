...
export function Hero({ onGetStarted }: HeroProps) {
  ...
  return (
    <div className="relative min-h-screen w-full font-sans select-none overflow-hidden flex flex-col justify-between">
      ...
      {/* FULL-SCREEN BACKGROUND IMAGES WITH TINTED OPACITY */}
      <div className="absolute inset-0 pointer-events-none z-0">
        {snackSlides.map((slide, idx) => (
          <div
            key={slide.image}
            className={`absolute inset-0 bg-cover bg-center transition-all duration-1000 ease-in-out ${
              idx === currentIndex ? 'opacity-80 scale-100' : 'opacity-0 scale-105'
            }`}
            style={{ backgroundImage: `url('${slide.image}')` }}
          />
        ))}

        {/* Global Light Shade to maintain text contrast without solid boxes */}
      ...
      {/* MAIN CENTER CONTENT (NO BACKGROUND BOX) */}
      <main className="relative z-10 max-w-4xl mx-auto px-6 py-12 flex-1 flex flex-col items-center justify-center text-center">
        ...
        <div className="min-h-[100px] sm:min-h-[120px] flex items-center justify-center">
          <h2 className="text-3xl sm:text-5xl lg:text-6xl font-black text-slate-950 tracking-tight leading-tight max-w-3xl transition-all duration-500 drop-shadow-md">
            {snackSlides[currentIndex].title}
          </h2>
        </div>

        {/* Dynamic Subtitle */}
        <p className="mt-4 text-sm sm:text-lg text-slate-900 max-w-xl font-bold leading-relaxed min-h-[60px] drop-shadow-sm">
