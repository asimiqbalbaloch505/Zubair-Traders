...
export function InvoiceModal({ sale, onClose, Modal, Button, money }: InvoiceModalProps) {
  ...
  return (
    <Modal title={`Invoice #${cleanInvoiceNo}`} eyebrow="Official Sales Invoice" onClose={onClose}>
      {/* Container wrapper ensuring the invoice stays centered in the visible viewport */}
      <div className="flex items-center justify-center my-auto min-h-[60vh]">
        <div className="printable-invoice relative w-full p-6 bg-white text-black rounded-lg border border-border overflow-hidden">
          ...
          {sale.items && sale.items.length > 0 ? (
            <div className="border border-gray-200 rounded overflow-hidden mb-4 max-h-[40vh] overflow-y-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-gray-100 border-b border-gray-200 text-black font-bold uppercase text-[10px] sticky top-0 bg-white">
                  <tr>
